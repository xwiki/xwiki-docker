...
-	`./gradlew submitOfficialImage` (post-push): regenerates the `library/xwiki` file and opens the Pull Request against [docker-library/official-images](https://github.com/docker-library/official-images) from your GitHub fork. It refuses to submit commits that are not reachable upstream (a dirty tree, a `HEAD` that is not on `master`, a Pull Request still open from a previous run) or images that the Docker Build workflow has not built successfully. Requires an authenticated `gh` CLI. Flags: `-PdryRun` generates the file, shows the diff and reports what those guards make of your current state, without opening a PR (it reports rather than fails, so you can preview before committing); `-PskipCiCheck` submits without a green Docker Build run (that workflow also fails on transient download errors); `-PtargetRepo=<you>/official-images` rehearses the whole write path against a repository of your own instead of submitting upstream.
